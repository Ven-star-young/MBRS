...
| 训练轮数 | 103 epochs | 103 epochs |
| 其他参数 | 同实验一 | 同实验一 |

### 2.3 训练过程与结果对比

两组实验均进行了三阶段训练（37 + 15 + 51 epochs），合并后各 103 轮。前两阶段共 52 轮训练后，模型在裁剪攻击下仍未能充分收敛；为进一步验证扩散块的有效性，追加了第三阶段训练（从 Epoch 50 续练至 Epoch 100），总计约 100 轮。

下图将无扩散块（蓝色）与有扩散块（红色）的训练曲线绘制在同一坐标系中，便于直观对比。

![实验二训练对比](results/experiment2_train_comparison.png)
*图 2.1: 实验二训练集指标对比（蓝：无扩散块，红：有扩散块）*

![实验二验证对比](results/experiment2_val_comparison.png)
*图 2.2: 实验二验证集指标对比（蓝：无扩散块，红：有扩散块）*

### 2.4 训练指标对比（Epoch 102，最终）

| 指标 | 无扩散块 | 有扩散块 | 优势方 |
|---|---|---|---|
| **训练 BER** | 9.03% | **3.51%** | 有扩散块 |
| **训练 PSNR** | **33.76 dB** | 31.48 dB | 无扩散块 |
| **训练 SSIM** | **0.839** | 0.778 | 无扩散块 |
| **验证 BER** | **9.45%** | 14.47% | 无扩散块 |
| **验证 PSNR** | **34.28 dB** | 31.66 dB | 无扩散块 |
| **验证 SSIM** | **0.834** | 0.778 | 无扩散块 |

历史最佳指标对比：

| 指标 | 无扩散块 | 有扩散块 | 优势方 |
|---|---|---|---|
| **训练最佳 BER** | 7.16% (Epoch 56) | **3.34%** (Epoch 99) | 有扩散块 |
| **验证最佳 BER** | **4.90%** (Epoch 20) | 5.32% (Epoch 15) | 无扩散块 |
| **训练最佳 SSIM** | **0.883** (Epoch 91) | 0.791 (Epoch 99) | 无扩散块 |
| **验证最佳 SSIM** | **0.894** (Epoch 84) | 0.789 (Epoch 91) | 无扩散块 |

### 2.5 测试结果对比

使用训练完成的模型（Epoch 100 权重），测试噪声层为 `Combined([JpegMask(50), Crop(0.15, 0.15)])`：

| 指标 | 无扩散块 | 有扩散块 |
|---|---|---|
| **BER (误码率)** | **18.25%** | 24.33% |
| **PSNR** | **34.70 dB** | 32.14 dB |
| **SSIM** | **0.8345** | 0.7795 |
...
引入裁剪攻击后，两项实验的 BER 均显著上升（与实验一 0.006% 相比，上升至 18%~25%）。这是因为 Crop(0.15, 0.15) 随机裁剪掉图像边缘 15% 的区域，直接导致该区域嵌入的水印信息完全丢失。对于 128×128 的图像，这意味着约 28% 的像素区域可能被丢弃，对仅嵌入 64 bits 的水印系统构成极大挑战。即便训练了 103 轮，误码率仍处于较高水平，说明裁剪攻击是数字水印面临的最严峻鲁棒性考验之一。

**2. 扩散块的训练效果——结论明确**

从训练集 BER 来看，有扩散块（最佳 3.34%，最终 3.51%）**持续且显著地优于**无扩散块（最佳 7.16%，最终 9.03%）。在 103 轮训练后，扩散块模型训练 BER 仅为无扩散块模型的约 39%。这一优势从 50 轮延续至 100 轮，充分说明了扩散块通过全连接层将每一位水印信息弥散到整张图像，有效缓解了裁剪造成的信息局部丢失问题，与理论预期完全一致。

**3. 验证与测试阶段——扩散块存在严重过拟合**

尽管训练集上扩散块优势巨大，但在验证集和测试集上表现相反：

- 验证集最佳 BER：无扩散块 4.90%（Epoch 20）vs 有扩散块 5.32%（Epoch 15），两者接近。
- 验证集最终 BER：无扩散块 9.45% vs 有扩散块 14.47%，扩散块模型在验证集上严重退化。
- 测试集 BER：无扩散块 18.25% vs 有扩散块 24.33%。

扩散块模型在训练和验证/测试之间的性能差距（泛化差距）远大于无扩散块模型——训练 BER 仅 3.51% 但测试 BER 高达 24.33%，差距达 20 个百分点。相比之下，无扩散块模型的泛化差距约为 9 个百分点。

分析过拟合的可能原因：
- 扩散块引入了额外的全连接层参数（$O(N^2)$），模型容量增大，在有限数据（10000 张训练图）下更容易过拟合训练集的特定噪声模式。
- 信息弥散策略虽然理论上增强了抗裁剪能力，但使得模型对训练集中每张图像的具体纹理分布产生了"记忆"——在训练时模型学会在特定纹理区域分散嵌入水印，但在测试集的不同纹理分布下泛化能力不足。
- 验证集 BER 从 Epoch 15 后持续上升（从 5.32% 退化到 14.47%），而无扩散块模型同样存在退化但幅度较小，说明扩散块对早停（early stopping）更为敏感。

**4. 图像质量**

无扩散块的 PSNR 和 SSIM 在 103 轮训练后均优于有扩散块（PSNR 34.70 dB vs 32.14 dB，SSIM 0.835 vs 0.780）。这是因为扩散块在整张图像上产生弥散式的嵌入纹理，相比局部嵌入更难保持完全的不可见性，与原论文的描述一致。值得一提的是，即使不加扩散块，103 轮训练也将 SSIM 从 50 轮时的 0.808 提升至 0.835，说明持续训练对图像质量有积极影响。
...
下图将实验一 SE Block（绿色）与实验三 Self-Attention（橙色）的训练曲线绘制在同一坐标系中，便于直观对比两种注意力机制的表现差异。

![实验三训练对比](results/experiment3_train_comparison.png)
*图 3.3: 实验三训练集指标对比（绿：SE Block，橙：Self-Attention）*

![实验三验证对比](results/experiment3_val_comparison.png)
*图 3.4: 实验三验证集指标对比（绿：SE Block，橙：Self-Attention）*
...
| 实验二 (a) | 无 Diffusion + Crop (103 epochs) | 18.25% | 34.70 | 0.8345 |
| 实验二 (b) | 有 Diffusion + Crop (103 epochs) | 24.33% | 32.14 | 0.7795 |
...
2. **裁剪攻击是严峻挑战，扩散块的训练优势与泛化劣势并存**：裁剪攻击使 BER 从 0.006% 飙升至 18% 以上。扩散块在训练阶段大幅降低了 BER（最佳 3.34% vs 无扩散块 7.16%），验证了其信息弥散机制的有效性。但更强的模型容量导致了更严重的过拟合：训练→测试 BER 差距达 20 个百分点（vs 无扩散块的 9 个百分点），最终测试 BER 甚至高于无扩散块。未来可通过增加训练数据量、增强数据多样性、减少扩散层宽度或引入正则化手段来改善泛化能力。

3. **Self-Attention 提升显著**：在训练和验证阶段，通道自注意力在所有指标上全面超越 SE Block。训练 BER 降低 36%，验证 BER 降低 68%，同时图像质量也有提升。测试阶段略逊于 SE Block，但其优秀的训练和验证表现表明自注意力机制在数字水印领域具有重要潜力，值得进一步研究。
