...
class Encoder_MP_SelfAttn(nn.Module):
	...
	def __init__(self, H, W, message_length, blocks=4, channels=64):
		...
		self.image_first_layer = selfSENet(channels, channels, blocks=blocks, block_type="selfBottleneckBlock")

		self.message_pre_layer = nn.Sequential(
			ConvBNRelu(1, channels),
			ExpandNet(channels, channels, blocks=message_convT_blocks),
			selfSENet(channels, channels, blocks=message_se_blocks, block_type="selfBottleneckBlock"),
		)

		self.message_first_layer = selfSENet(channels, channels, blocks=blocks, block_type="selfBottleneckBlock")
